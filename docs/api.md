...
`GET /v1/search?q=…&types=note,task,topic,capture&limit=20&all=1` → `[{"id","type","title","score","preview","kind","state"}]`. Archived objects and captures are excluded unless `all=1`. With an embedding model configured the ranking fuses the lexical index with the nearest vectors (reciprocal rank fusion); the conversation's search tool and the triage context use the same search.
...
| `GET /v1/health` | Adds `timezone`, `ui` (embedded web UI present) and `warnings[]` (missing API keys, recovered log). 0.3.5 adds `dictation` (a usable transcription provider is configured), 0.3.6 `research` (a model and a search backend are configured), 0.3.7 `embedding` (semantic search on) and `maintenance{enabled, running, next}`. |
...
| tasks | 0.3.8: tasks carry `kind` (`""` or `"research"`). Triage sets it from its classification in any language; `task.create` accepts `kind: "research"`; research starts for such tasks by itself unless `research.manual` is set, and the client offers "Research this" on them. No text prefix is involved. |
...
| `POST /v1/research` | `{task_id}` (an open task, usually one that starts with "Research:") or `{question, topics?}` (files such a task first) → 202 `{task_id, status: "running"}`. The reader searches and reads the web in the background (ADR-0010: no write tools), then the curator stores one note per task: the answer in an `external` callout with `[n]` citations, a findings list, `[[src_…]]` source lines with retrieval dates, open questions, and a footer naming model and backend; each cited page becomes a `source` object; the task is completed with a link to the note. All of it is one transaction, so one undo removes it. Errors: 409 `already_running` / `not_open`, 503 `research_unavailable`. Progress arrives as SSE `research.progress` events `{task_id, step: search|fetch|read|store|done|error, summary, note_id?, sources?}`. |
| `GET /v1/research` | `{running: [task ids], available: bool}`. |
| `GET /v1/maintenance` | `{enabled, running, run_id?, next, last: Run, runs: [Run…]}` where a Run is `{id, trigger: schedule|manual, started, finished, jobs: [{name: integrity|untagged|duplicates|summaries|assist, checked, changed, proposed, notes[], txn_ids[], error?, skipped?}], error?}`. History lives under `<data_dir>/maintenance`, not in the event log. |
| `POST /v1/maintenance/run` | Starts a run now → 202 `{run_id, status: "running"}`, 409 `already_running`. Progress streams as SSE `maintenance.progress` `{run_id, job, summary, done}`. Changes made by a run carry the cause `maintenance/<run_id>` and the actors `system:maintenance` (deterministic fixes) or `llm:maintenance/<provider>/<model>`; everything that would merge or delete arrives in the inbox as a proposal capture (`source: "maintenance"`, `result.reason: "proposal"`, `result.lines[]`, `result.core_proposal`) that `POST /v1/captures/{id}/accept` applies as the user (ADR-0014). |
