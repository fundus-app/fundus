...
- Providers are OpenAI-compatible endpoints: OpenAI, Google Gemini (its `/v1beta/openai` compatibility layer), Anthropic, OpenRouter and Ollama (any address, no key). Each carries a `transcription` mode so dictation can use the audio endpoint (OpenAI), a chat completion with an `input_audio` part (Gemini, OpenRouter) or nothing (Anthropic, Ollama). Model suggestions prefer the newest generation and fall back to version parsing so new families need no release.
- Dictation is transcription only: the recording goes to the dictation provider, the text comes back into the capture field for review, nothing is stored until the user captures it. Audio is not kept.
