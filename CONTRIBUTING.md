...
| Live provider | `FUNDUS_LIVE_TESTS=1 OPENAI_API_KEY=… go test ./internal/triage ./internal/llm ./internal/research ./internal/maintenance -run Live -v` | Real captures through a real model: task with resolved due date, a vague idea that must not become a task, "hm" parked or dismissed, three captures about one new project that must end up in one topic; plus a spoken sentence (espeak-ng) through the transcription endpoint, plus one real research task (`./internal/research`, OpenAI search + reader) and one maintenance run with real embeddings (`./internal/maintenance`). Costs a few cents; run before a release. |
...
Flatpak and snap are built only in the release workflow, from the same Flutter bundle and daemon as the AppImage: `deploy/flatpak/io.github.fundus_app.fundus.yml` (GNOME 49 runtime, `flatpak-builder` in the Flathub container) and `snap/snapcraft.yaml` (core24, strict, GNOME extension, `snapcore/action-build`). Both take their binaries from the `linux-bundle` artifact, so nothing is compiled twice. To publish the snap to the Snap Store, register the name once (`snapcraft register fundus`), export a login (`snapcraft export-login --snaps fundus --channels edge,stable -`) and store it as the repository secret `SNAPCRAFT_STORE_CREDENTIALS`; pushes to `main` then land on the `edge` channel and version tags on `stable`. A Flathub submission needs its own repository under `flathub/` with an offline manifest that builds from source; the metainfo and desktop file in `deploy/flatpak/` are ready for it.
