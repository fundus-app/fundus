...
- research: the user explicitly asks you to research, look up or find out something that the notes cannot answer; create a task whose text is the research question in the capture's own words and language, with "kind": "research". Do not add a prefix or a label to the text.
...
5. Never invent due dates, priorities or effort. Set them only when the capture states them. Resolve relative dates ("tomorrow", "next Friday") using the current date given in the context; a date without a year ("by 15 October") means its next occurrence from today, even when the text mentions another year for a different reason ("the 2025 tax return").
